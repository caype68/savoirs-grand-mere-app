...
async function extractRemediesWithGPT(text, chunkIndex) {
...
Réponds UNIQUEMENT avec le tableau JSON, sans markdown ni explication.`;
  ...
  try {
    ...
    console.error(`❌ Erreur extraction chunk ${chunkIndex}:`, error.message);
    return [];
  }
}
...
function chunkText(text, maxChunkSize = 6000) {
  ...
  // Découper directement par taille fixe
...
async function processAndSaveRemedies(pdfPath, outputPath) {
  ...
  const remedesFormatted = allRemedies.map((r, index) => ({
    ...
    posologie: r.posologie || { dose: '', frequence: '', duree: '', moment: '' },
